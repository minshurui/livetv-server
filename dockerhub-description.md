...
镜像发布必须先通过同一提交的 Huya/Douyu 实播冒烟与双架构构建验证。播放接口使用 302 临时跳转并禁止缓存；旧白名单不会隐藏新目录中的房间。换台时取消旧 CDN 连接，续流地址过期后重新解析。短时测试通过不等于所有频道都能长期无卡顿。

直播目录不是永久快照：容器启动后默认每 15 分钟同步虎牙/斗鱼开播状态，播放签名默认 30 秒短缓存。播放器若只在启动时加载一次 M3U，仍需开启列表自动刷新或手动重新加载，才能看到新开播/下播频道。

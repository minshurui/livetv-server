...
IPTV_CHANNEL_SCOPE="${IPTV_CHANNEL_SCOPE:-all}"
IPTV_VERIFY_STREAMS="${IPTV_VERIFY_STREAMS:-1}"
IPTV_VERIFY_TIMEOUT="${IPTV_VERIFY_TIMEOUT:-12}"
IPTV_VERIFY_WORKERS="${IPTV_VERIFY_WORKERS:-6}"
IPTV_URLS_PER_CHANNEL="${IPTV_URLS_PER_CHANNEL:-2}"
IPTV_VERIFY_FALLBACK="${IPTV_VERIFY_FALLBACK:-1}"
...
export IPTV_CHANNEL_SCOPE IPTV_VERIFY_STREAMS IPTV_VERIFY_TIMEOUT IPTV_VERIFY_WORKERS
export IPTV_URLS_PER_CHANNEL IPTV_VERIFY_FALLBACK
...
case "$IPTV_CHANNEL_SCOPE" in
  all|cctv_satellite) ;;
  *) echo "[entry] IPTV_CHANNEL_SCOPE 只能是 all 或 cctv_satellite" >&2; exit 2 ;;
esac
case "$IPTV_VERIFY_STREAMS" in
  0|1) ;;
  *) echo "[entry] IPTV_VERIFY_STREAMS 只能是 0 或 1" >&2; exit 2 ;;
esac
case "$IPTV_VERIFY_FALLBACK" in
  0|1) ;;
  *) echo "[entry] IPTV_VERIFY_FALLBACK 只能是 0 或 1" >&2; exit 2 ;;
esac

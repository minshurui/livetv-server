...
2. 虎牙条目的 URL 指向 `http://服务器:19090/stream/huya/房间号`。
3. Go 解析器读取结构化房间数据，按 2026 签名算法生成带时效的 H.264 FLV 地址。
4. 代理检查上游不是 HTML 错误页、空响应或测试回退视频。
5. CDN 断开时，代理重新签名并在同一客户端连接内从关键帧续接，同时修正 FLV 时间戳。
6. 房间下播或所有重试失败时返回明确错误，不再跳到测试录像。

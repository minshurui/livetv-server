...
白名单以 `douyu-alive.txt.json` 单文件原子发布，记录被检查的房间集合摘要与检测开始时间。超过 5 分钟、房间集合变化、文件缺失或损坏时，程序回退到当前目录，不让旧白名单隐藏新开播频道。耗时超过有效窗口的全量检测只作历史结果，不能被标记为“刚刚检测”。旧 txt/stamp 文件继续输出，兼容外部脚本。
...
正常情况下只有全部通过才会用临时文件原子替换 `.iptv-result.m3u`。若本地
FFmpeg 对所有候选都失败，默认熔断该探测器并使用 iptv-api 已测速的新结果，
防止已经过期的 last-good 永远不再更新。两条路径都以原子方式发布，不会先清空
旧文件。
...
| 任务 | 周期/触发 |
|---|---|
| 虎牙/斗鱼目录同步 | 启动时一次，之后默认每 15 分钟（`SYNC_CHANNELS_MINUTES` 可调） |
...
| IPTV 测速后地址失效 | 最终发布前 FFmpeg 解码首帧；按频道限制已验证线路数 | 播放后再次失效仍由客户端切换备用线路 |
| 本地 FFmpeg 探测器全失败 | 默认熔断探测器并发布 iptv-api 最新测速结果，下周期重试 | 降级周期内可能混入上游误判的坏源；可关闭熔断 |
| 需要精简为央视卫视 | 可选 `cctv_satellite` 模式按规范化频道名重新识别并重写分组 | 新增卫视频道名称需要更新识别表；默认 `all` 不受影响 |
| 写文件中途退出 | 临时文件完成后原子替换 | 磁盘损坏不在应用层解决范围 |
| 子进程崩溃 | `entry.sh` 每 20 秒守护重启 | 持续崩溃仍需根据日志修复根因 |
| CDN 地址过期/房间状态变化 | 默认 30 秒短缓存；302 临时跳转与列表/离线响应禁止缓存 | 非秒级在线状态推送，平台接口变更仍需适配 |

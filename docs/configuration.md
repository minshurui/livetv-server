...
| `HUYA_CACHE_TTL` | `30` | 虎牙成功解析地址缓存秒数；允许范围 5–600。过大可能保留停播状态 |
| `DOUYU_CACHE_TTL` | `30` | 斗鱼成功解析地址缓存秒数；允许范围 5–600。过大可能保留停播状态 |
| `SYNC_CHANNELS_MINUTES` | `15` | 虎牙/斗鱼直播目录刷新间隔；允许范围 5–59 |
| `PREWARM_PER_GROUP` | `2` | 加载 M3U 后每个平台每组后台预解析数量；`0` 关闭 |
| `PREWARM_MAX_CHANNELS` | `16` | 每个平台一次最多预解析频道数 |
| `PREWARM_WORKERS` | `3` | 后台预解析并发数 |
| `STREAM_READ_IDLE_SECONDS` | `10` | CDN 网络读取无数据的超时，范围 2–120 秒；持续传输不受总时长限制 |

播放器断开后，对应 CDN 拉流和重连等待随之取消；共享签名解析不会因单个观众离开而中断其他观众，仍受解析请求超时限制。续流预取的地址遵守平台缓存 TTL，过期会重新解析。网络超时触发重连不代表能够保证肉眼无卡顿。
...
| `SYNC_CHANNELS_MINUTES` | `15` | 容器内虎牙/斗鱼目录刷新间隔，允许 5–59 分钟 |

容器启动时同步一次，之后默认每 15 分钟同步；可通过 `SYNC_CHANNELS_MINUTES` 调整。虎牙热门总榜当前只抓前 15 页，因此会再按分类接口完整补抓“一起看”；房间号会自动去重。虎牙和斗鱼默认精简为“一起看、网游电竞、手游、单机、娱乐、户外、体育、其他”8 个组，最终 M3U 中会显示为 `虎牙·一起看`、`斗鱼·手游` 等名称。某个目录成功页数不足 80% 时保留旧文件，避免一次网络故障把数千条频道缩成几十条。最低数量保护仍不能代替真实拉流检测。

## 代理和 HLS 生命周期

目录同步有进程互斥锁，同一输出文件的启动同步和定时任务不会重叠；单次任务超过 240 秒退出，日志记录失败，保留最后成功目录。下个周期会重试。异常退出可能留下未发布的 `.channels-*.tmp` 文件，不影响正在使用的目录。

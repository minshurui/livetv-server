...
直播目录默认每 15 分钟重新同步；虎牙/斗鱼播放签名默认最多缓存 30 秒，并且使用 `302 + no-store` 临时跳转。播放器如果只加载一次 M3U，不会自动出现新开播房间，请开启播放列表自动刷新或重新加载列表。需要更快状态更新时，可在 compose 的环境变量中设置 `SYNC_CHANNELS_MINUTES=5`、`HUYA_CACHE_TTL=5`、`DOUYU_CACHE_TTL=5`。

发布门禁：同一提交的单元/状态回归测试 → 虎牙与斗鱼真实播放冒烟 → amd64/arm64 构建验证 → Docker Hub/阿里云发布。手动构建也不能跳过冒烟。CI 每平台至少一个候选实播 60 秒，并进行 60 秒解码/冻结/循环检测；这不等于所有频道和所有家庭网络长期无卡顿。

...
直播目录默认每 15 分钟重新同步；虎牙/斗鱼播放签名默认最多缓存 30 秒，并且解析跳转禁止客户端缓存。播放器如果只加载一次 M3U，不会自动出现新开播房间，请开启播放列表自动刷新或重新加载列表。需要更快状态更新时，可在 compose 的环境变量中设置 `SYNC_CHANNELS_MINUTES=5`、`HUYA_CACHE_TTL=5`、`DOUYU_CACHE_TTL=5`。
